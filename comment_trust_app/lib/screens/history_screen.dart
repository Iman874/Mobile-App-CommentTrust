...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  Future<void> _loadLastViewed() async {
    ...
    // If there is no saved history, we may show demo dummy only when the app is in demo mode
    // and the user has not completed any login (no token saved).
    if (last == null) {
      final token = await AuthService.token;
      if (ApiConfig.I.demoMode && (token == null || token.isEmpty)) {
        // Load a dummy product as the last viewed when we're in demo mode and no auth token exists.
        try {
      ...
      // No history and no demo fallback
      if (!mounted) return;
      setState(() {
